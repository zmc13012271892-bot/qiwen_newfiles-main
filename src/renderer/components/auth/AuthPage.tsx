...
export const AuthPage: React.FC = () => {
  ...
  const [form, setForm] = useState<FormState>({
    ...
    displayName: '', emailOrUsername: '', rememberMe: true,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        await dispatch(loginUser({ emailOrUsername: form.emailOrUsername, password: form.password, rememberMe: form.rememberMe })).unwrap();
      } else if (mode === 'register') {
        await dispatch(registerUser({ email: form.email, username: form.username, password: form.password, displayName: form.displayName })).unwrap();
        ...
        setSuccess('若该邮箱已注册，重置链接已发送，请查收邮件');
      }
    } catch (err: any) {
    ...
    <div style={{
      ...
      >
        ...
        <h1 style={{
          ...
          fontFamily: '"Noto Serif SC", serif', color: '#e8e6e0',
          marginBottom: 12,
        }}>启 文</h1>
        ...
        <AnimatePresence>
          ...
          >
            ...
            <p style={{ fontSize: 13.5, color: 'rgba(155,152,144,0.8)', marginBottom: 32 }}>
              {mode === 'login' ? '登录以继续您的创作' : mode === 'register' ? '免费开始使用启文' : '输入注册邮箱以接收重置链接'}
            </p>
              ...
              {(error || success) && (
                <motion.div
                  ...
                  style={{
                    ...
                    background: error ? 'rgba(255,107,107,0.08)' : 'rgba(82,201,122,0.08)',
                    border: `0.5px solid ${error ? 'rgba(255,107,107,0.25)' : 'rgba(82,201,122,0.25)'}`,
                    color: error ? '#ff6b6b' : '#52c97a',
                  }}
                >
                  {error || success}
                </motion.div>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {mode === 'register' && (
                <>
                  <InputField label="昵称" placeholder="如何称呼您？" value={form.displayName} onChange={set('displayName')} />
                  <InputField label="用户名" placeholder="3-20位，字母/数字/下划线" value={form.username} onChange={set('username')} />
                  <InputField label="邮箱" type="email" placeholder="your@email.com" value={form.email} onChange={set('email')} />
                </>
              )}

              {mode === 'login' && (
                <InputField label="邮箱或用户名" placeholder="输入邮箱或用户名" value={form.emailOrUsername} onChange={set('emailOrUsername')} />
              )}

              {mode === 'forgot' && (
                <InputField label="注册邮箱" type="email" placeholder="your@email.com" value={form.email} onChange={set('email')} />
              )}
              ...
              {mode === 'register' && (
                <InputField
                  label="确认密码" type={showPassword ? 'text' : 'password'}
                  placeholder="再次输入密码" value={form.confirmPassword} onChange={set('confirmPassword')}
                />
              )}
              ...
              {mode === 'login' && (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <button type="button" onClick={() => { setMode('forgot'); clearMessages(); }}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, color: 'rgba(200,169,110,0.7)' }}>
                    忘记密码？
                  </button>
                </div>
              )}
            ...
            <div style={{ marginTop: 28, paddingTop: 20, borderTop: '0.5px solid rgba(255,255,255,0.06)', textAlign: 'center' }}>
              <button
                onClick={() => dispatch({ type: 'auth/setLocalMode' })}
                style={{
